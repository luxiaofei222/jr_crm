...
function zhankai_menu(id,course_id){
	...
	if(type=="fa fa-plus"){//切换符号
		...
		$.get("/extend/get_sub_menu_list.jr",{
			'course_id':course_id,
			'id':id
		},function(data){
...
function coursemenu_jump_page(page){
	jiazaidonghua();
	$.post("/extend/get_course_menu.jr",{
		'pageNumber' : page,
		'limit' : 10
	},function(data){
...
//弹出添加推广页面
function to_add_extend(course_id){
	layer.open({
		  type: 2,
		  title: ['添加推广信息'],
		  area: ['1100px', '850px'],
		  shadeClose: false, //点击遮罩关闭
		  content: "/extend/to_add_extend.jr?course_id="+course_id
		  });
}

function to_add_phone_extend(course_id){
	layer.open({
		  type: 2,
		  title: ['添加手机封面'],
		  area: ['900px', '400px'],
		  shadeClose: false, //点击遮罩关闭
		  content: "/extend/to_add_phone_extend.jr?course_id="+course_id
		  });
}
//去修改meta信息
function to_update_meta(course_id){
	layer.open({
		  type: 2,
		  title: ['修改META信息'],
		  area: ['900px', '300px'],
		  shadeClose: false, //点击遮罩关闭
		  content: "/extend/to_update_meta.jr?course_id="+course_id
		  });
}
//信息弹窗
