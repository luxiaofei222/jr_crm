...
<script>
//添加联系人信息信息
function save_question_course(){
	if(check_question_course_name()){
		 $("#myform").ajaxSubmit({
				type : 'POST',
...
//检查题库课程分类信息
function check_question_course_name(){
	var bol = false;
...
<form class="form-horizontal add_qiye_dialog" enctype="multipart/form-data" id="myform" style="padding:21px;">
      ...
      <div class="col-xs-8">
            <select data-toggle="select" id="course_parent_id" name="course_parent_id" onchange="get_sub_course()"  class="form-control select select-primary">
    ...
    <div class="form-group" style="margin-top:15px;">
      ...
      <div class="col-xs-8">
        <input class="form-control" onblur="check_question_course_name()" name="question_course_name" id="question_course_name" placeholder="输入题库类别">         
      </div>
    </div>
    <div class="form-group" style="margin-top:15px;">
      ...
      <div class="col-xs-8">
        <textarea class="form-control" rows="3" name="question_course_dis" id="question_course_dis" placeholder="请输入课程分类描述" style="color:#444;"></textarea>         
          ...
          <button type="button" onclick="save_question_course()" class="btn btn-primary btn-lg" style="width:100px;">提交</button>
          </div>
      	</div>
	</form>	