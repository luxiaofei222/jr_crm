...
$(function() {
	...
	// 2、src属性需要像下面的方式修改，上面的两种方式修改，无效；
...
function update_teacher(teacher_id){
	if(check_teacher_name()&&check_teacher_dis()){
		$("#myform").ajaxSubmit({
			type : 'POST',
			url : "/back_teacher/update_teacher.jr?teacher_id="+teacher_id,
...
<form class="form-horizontal" enctype="multipart/form-data" id="myform" style="padding:20px;">
    <div class="form-group" style="margin-top:15px;">
      ...
      <div class="col-xs-8">
        <input class="form-control" value="${teacher.teacher_name }" onblur="check_teacher_name()" id="teacher_name" name="teacher_name" placeholder="输入教师姓名">         
      </div>
    </div>
    <div class="form-group" style="margin-top:15px;">
      <label class="col-xs-2 control-label right_wz">教师称号：</label>
      <div class="col-xs-8">
        <input class="form-control" value="${teacher.teacher_course }" id="teacher_course" name="teacher_course" placeholder="输入教师姓名">         
      </div>
    </div>
    <div class="form-group" style="margin-top:15px;">
      ...
      <div class="col-xs-8">
            ...
            <c:if test="${teacher.teacher_sex =='男'}"> 
			<option value="${teacher.teacher_sex }">${teacher.teacher_sex }</option>
              <option value="女">女</option>
              </c:if> 
               <c:if test="${teacher.teacher_sex =='女'}"> 
			<option value="${teacher.teacher_sex }">${teacher.teacher_sex }</option>
    ...
    <div class="form-group" style="margin-top:15px;">
      ...
      <div class="col-xs-8">
        <input class="form-control" value="${teacher.teacher_phone }" id="teacher_phone" name="teacher_phone" placeholder="输入教师电话">         
      </div>
    </div>
    <div class="form-group" style="margin-top:15px;">
      ...
      <div class="col-xs-8">
            <select data-toggle="select" onchange="get_sub_couerse()"name="course_parent_id"  id="parent_id" class="form-control select select-primary">
              ...
              <c:if test="${courseMenu_dan.course_name == course_menu.course_name}">
    ...
    <div class="form-group" style="margin-top:15px;">
      ...
      <div class="col-xs-8">
            <select data-toggle="select" id="course_id" name="course_id" class="form-control select select-primary">
              <c:forEach items="${courseMenus_sub }" var="course_sub">
              <c:if test="${courseMenu_sub.course_name== course_sub.course_name}">
              <option selected="selected" value="${course_sub.course_id }">${course_sub.course_name }</option>
              </c:if>
               <c:if test="${courseMenu_sub.course_name!= course_sub.course_name}">
	   ...
	   <div class="form-group" style="margin-top:15px;">
  		...
  		<div class="col-xs-8 clearfix">
				<div class="pull-left">
  				<img id="preview" src="${teacher.teacher_pic }" width="84" height="84" style="background:url('/images/school/back/teacher/touxbg.png') no-repeat">
  			</div>
    ...
    <div class="form-group" style="margin-top:15px;">
      ...
      <div class="col-xs-8">
        <textarea class="form-control" id="teacher_dis"  oninput="tishi_cotent()" name="teacher_dis" rows="3" placeholder="请输入教师简介，不超过40字" style="color:#444;">${teacher.teacher_dis }</textarea>         
      </div>
    </div>
    <div class="form-group" style="margin-top:15px;">
      <div class="col-xs-2"></div>
       <p class="col-xs-8" id="tishi">请输入教师简介！</p>
    </div>
      	<div class="form-group" style="margin-top:15px;">
          ...
          <button type="button" onclick="update_teacher(${teacher.teacher_id})" class="btn btn-primary btn-lg" style="width:100px;">保存</button>
          </div>
      	</div>
	</form>	