...
<script>
//添加成考学校招生信息
function to_add_chengkao_xuexiao(){
	 layer.open({
		  type: 2,
		  title: ['成考招生'],
		  area: ['1000px', '700px'],
		  shadeClose: true, //点击遮罩关闭
		  content: "/ck_zhaosheng/get_add_chengkao_info.jr"
		  });
}
//修改学校信息
function to_update_ck_xuexiao(zhaosheng_id){
	 layer.open({
		  type: 2,
		  title: ['修改成考招生信息'],
		  area: ['1000px', '700px'],
		  shadeClose: true, //点击遮罩关闭
		  content: "/ck_zhaosheng/to_update_ck_xuexiao.jr?zhaosheng_id="+zhaosheng_id
		  });
}
//学校列表翻页-上一页 下一页
function ckxuexiao_jump_page(page){
	jiazaidonghua();
	...
	<div class="right_content">
		<div class="operation">
			<div class="opera_left left">
				 <button type="button" onclick="to_add_chengkao_xuexiao()" class="btn left btn-primary" id="add_role">
					...
					<c:if test="${not empty ckZhaoShengs.list }">
						<tbody>
							<c:forEach items="${ckZhaoShengs.list }" var="ckzs" varStatus="vs">
								...
								<td>
									<button type="button" onclick="to_update_ck_xuexiao(${ckzs.zhaosheng_id})" class="btn btn-success btn-xs">修改</button>
		...
		<div class="pages">
			...
			<ul class="pagination right">
				<c:if test="${ckZhaoShengs.hasPreviousPage==true}">
					<li class="previous"
						onclick="ckxuexiao_jump_page(${ckZhaoShengs.pageNumber-1})"><a
				...
				<c:forEach items="${ckZhaoShengs.navigatePageNumbers }" var="page">
					<c:choose>
						<c:when test="${ckZhaoShengs.pageNumber==page}">
							<li class="active" onclick="ckxuexiao_jump_page(${page})"><a
								href="#fakelink">${page}</a></li>
						</c:when>
						<c:otherwise>
							<li onclick="ckxuexiao_jump_page(${page})"><a href="#fakelink">${page}</a></li>
						</c:otherwise>
					</c:choose>
				</c:forEach>
				<c:if test="${ckZhaoShengs.hasNextPage==true}">
					<li class="next" onclick="ckxuexiao_jump_page(${ckZhaoShengs.pageNumber+1})"><a
