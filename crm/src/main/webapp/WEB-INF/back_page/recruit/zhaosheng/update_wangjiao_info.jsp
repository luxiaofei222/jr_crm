...
<script>
	...
	//添加学校介绍
	function update_chengkao_xuexiao(obj,zhaosheng_id) {
		var markupStr = $('#summernote').summernote('code');
		if (markupStr != null && markupStr != "<p><br></p>") {
			$("#myform").ajaxSubmit({
				type : 'POST',
				data : {
					'yuanxiao_jieshao' : markupStr,
					'zhaosheng_id':zhaosheng_id
				},
				url : "/wj_zhaosheng/update_wangjiao_xuexiao.jr",
...
<div class="tianjia_xuexiao">
	<form class="form-horizontal" enctype="multipart/form-data" id="myform"
		...
		<div class="form-group">
			...
			<div class="col-xs-10">
				<select id="wangjiao_id" name="wangjiao_id">
					<c:forEach items="${universities }" var="wangjiao">
						<c:if test="${ckZhaoSheng.wangjiao_id==wangjiao.university_id }">
							<option selected="selected" value="${wangjiao.university_id }">${wangjiao.university_name }</option>
						</c:if>
						<c:if test="${ckZhaoSheng.wangjiao_id!=wangjiao.university_id }">
							<option value="${wangjiao.university_id }">${wangjiao.university_name }</option>
						</c:if>
					</c:forEach>
				</select>
			</div>
		...
		<div class="form-group">
			...
			<div class="col-xs-10">
				<select id="zhaosheng_state" name="zhaosheng_state">
					<c:if test="${ckZhaoSheng.zhaosheng_state ==0}">
						<option value="0">正在招生</option>
						<option value="1">停止招生</option>
						<option value="2">即将招生</option>
					</c:if>
					<c:if test="${ckZhaoSheng.zhaosheng_state ==1}">
						<option value="1">停止招生</option>
						<option value="0">正在招生</option>
						<option value="2">即将招生</option>
					</c:if>
					<c:if test="${ckZhaoSheng.zhaosheng_state ==2}">
						<option value="2">即将招生</option>
						<option value="0">正在招生</option>
						<option value="1">停止招生</option>
					</c:if>
      ...
      <div class="col-xs-10">
        <input type="text" id="remen_zhuanye" value="${ckZhaoSheng.remen_zhuanye }" name="remen_zhuanye" />
      </div>
    </div>
    <div class="form-group">
      <label class="col-xs-2 control-label">校训：</label>
      <div class="col-xs-10">
        <input type="text" id="xiaoxun"  value="${ckZhaoSheng.xiaoxun }" name="xiaoxun" />
      </div>
    </div>
		<div class="form-group">
			...
			<div class="col-xs-10">
				<ul class="pics">
					...
					<li>
						<div class="xueyuan_pic">
							<img id="preview4"  src="${ckZhaoSheng.fugai_quyu }" width="185" height="150" />
						</div>
						<div class="pic_btn">
							<p>学校覆盖区域</p>
							<a href="javascript:void(0)" class="btn btn-info btn-xs"
								style="position: relative;">浏览 <input type="file"
								id="upfile4" name="fugai_str"
								style="position: absolute; opacity: 0; top: 0px; left: 0px; width: 100%; height: 100%;" />
							</a>
						</div>
					</li>
					<li>
						<div class="xueyuan_pic">
							<img id="preview2" src="${ckZhaoSheng.xuelizhengshu }" width="220" height="150" />
						</div>
						<div class="pic_btn">
							<p>学历证书样本</p>
							<a href="javascript:void(0)" class="btn btn-info btn-xs"
								style="position: relative;">浏览 <input type="file"
								id="upfile2" name="xueli_str"
								style="position: absolute; opacity: 0; top: 0px; left: 0px; width: 100%; height: 100%;" />
							</a>
						</div>
					</li>
					<li>
						<div class="xueyuan_pic">
							<img id="preview3" src="${ckZhaoSheng.xuewei_zhengshu }" width="220" height="150" />
						</div>
						<div class="pic_btn">
							<p>学位证书样本</p>
							<a href="javascript:void(0)" class="btn btn-info btn-xs"
								style="position: relative;">浏览 <input type="file"
								id="upfile3" name="xuewei_str"
								style="position: absolute; opacity: 0; top: 0px; left: 0px; width: 100%; height: 100%;" />
							</a>
						</div>
					</li>
					<li>
						<div class="xueyuan_pic">
							<img id="preview5" src="${ckZhaoSheng.xiangqing_beijing }" width="705" height="150" />
						</div>
						<div class="pic_btn">
							<p>详情页背景图</p>
							<a href="javascript:void(0)" class="btn btn-info btn-xs"
								style="position: relative;">浏览 <input type="file"
								id="upfile5" name="xiangqing_str"
								style="position: absolute; opacity: 0; top: 0px; left: 0px; width: 100%; height: 100%;" />
							</a>
						</div>
					</li>
		...
		<div class="form-group">
			...
			<div class="col-xs-10">
				<div id="summernote">${ckZhaoSheng.yuanxiao_jieshao }</div>
			</div>
		</div>
		<input type="button" onclick="update_chengkao_xuexiao(this,${ckZhaoSheng.zhaosheng_id})" value="提交">
	</form>
</div>