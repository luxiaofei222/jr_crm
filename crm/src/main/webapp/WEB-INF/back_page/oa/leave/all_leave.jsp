...
//翻页上一页下一页
function leave_jump_page(page,limit,employee_id ,leave_type,bumen_id,nianyue ){
	jiazaidonghua();
	$.post("/back_leave/get_all_bumen_leave.jr",{
...
//跳转页面
function leave_tiaozhuan_page(page,limit,employee_id ,leave_type,bumen_id,nianyue){
...
function shaixuan_tioajian(obj,limit){
		...
		$("#myform").ajaxSubmit({
			type : 'POST',
			url : "/back_leave/get_all_bumen_leave.jr",
			data:{
				'pageNumber':1,
				'limit':limit
			},
...
//查看任务详情
function to_check_leave(leave_id,type){
	 layer.open({
		  type: 2,
		  title: ['请假审批'],
		  area: ['850px', '600px'],
		  shadeClose: false, //点击遮罩关闭
		  content: "/back_leave/to_check_oa_leave.jr?leave_id="+leave_id+"&type="+type
		  });
}
...
<div class="back_right">
	...
	<div class="right_content">
		<form enctype="multipart/form-data" id="myform"
			...
			<div class="form-group clearfix"
				style="padding-top: 10px; margin-bottom: 5px;">
				<label class="col-xs-1 right_wz">月份：</label>
				<div class="col-xs-2">
					<select class="form-control" name="nianyue" id="nianyue"
						style="line-height: 35px; height: 35px; padding: 0px;">
						<option value="">请选择</option>
						<c:forEach items="${time_list }" var="nian_yue">
						<option value="${nian_yue }">${nian_yue }</option>
						</c:forEach>
					</select>
				</div>
				...
				<c:if test="${not empty backLeaves.list }">
					<tbody>
						<c:forEach items="${backLeaves.list }" var="leave" varStatus="vs">
							...
							<c:if test="${leave.leave_type==4 }">
							产假
							</c:if>
							<c:if test="${leave.leave_type==5 }">
							丧假
							</c:if>
							<c:if test="${leave.leave_type==6 }">
							其他
							</c:if>
							<c:if test="${leave.leave_type==7 }">
							倒休
							</c:if>
							<c:if test="${leave.leave_type==8 }">
							年假
							</c:if>
							</td>
							<td>
							...
							<td><fmt:formatDate  type="both"
									value="${leave.leave_time}" /></td>
							<td>
							<c:if test="${leave.leave_state==0}">
								经理审批中
							</c:if>
							<c:if test="${leave.leave_state==1}">
								总监审批中
							</c:if>
							<c:if test="${leave.leave_state==2}">
								总经理审批中
							</c:if>
							...
							<c:if test="${leave.leave_state==4}">
								已销假
							</c:if>
							<c:if test="${leave.leave_state==5}">
								未批准
							</c:if>
							</td>
							<td>
								<button type="button"
										onclick="to_check_leave(${leave.leave_id },1)"
			...
			<div class="whole left">
				<div class="go_page">
					<span>第</span><input type="text" id="page_num" /> <span>页</span> <a
						...
						onclick="leave_tiaozhuan_page(${backLeaves.pages},${limit},'${employee_id }','${leave_type }','${bumen_id }','${nianyue }')"
						class="btn_go">GO</a>
				</div>
			</div>
			<ul class="pagination right">
				<li><a href="javascript:void(0)"
					onclick="leave_jump_page(1,${limit },'${employee_id }','${leave_type }','${bumen_id }','${nianyue }')"
					class="fa fa-fast-backward"></a></li>
				<c:if test="${backLeaves.hasPreviousPage==true}">
					<li class="previous"
						onclick="leave_jump_page(${backLeaves.pageNumber-1},${limit },'${employee_id }','${leave_type }','${bumen_id }','${nianyue }')"><a
				...
				<c:forEach items="${backLeaves.navigatePageNumbers }" var="page">
					<c:choose>
						...
						<c:otherwise>
							<li
								onclick="leave_jump_page(${page},${limit },'${employee_id }','${leave_type }','${bumen_id }','${nianyue }')"><a
				...
				<li class="pagination-dropdown dropup"><a href="#fakelink"
					...
					<ul class="dropdown-menu">
						<li><a href="#fakelink"
							onclick="leave_jump_page(${backLeaves.pageNumber},20,'${employee_id }','${leave_type }','${bumen_id }','${nianyue }')">20</a></li>
						<li><a href="#fakelink"
							onclick="leave_jump_page(${backLeaves.pageNumber},50,'${employee_id }','${leave_type }','${bumen_id }','${nianyue }')">50</a></li>
						<li><a href="#fakelink"
							onclick="leave_jump_page(${backLeaves.pageNumber},100,'${employee_id }','${leave_type }','${bumen_id }','${nianyue }')">100</a></li>
					</ul></li>
				...
				<li
					onclick="leave_jump_page(${backLeaves.pages},${limit },'${employee_id }','${leave_type }','${bumen_id }','${nianyue }')"><a
