...
<script>
	...
	function tijiao_positive(positive_id){
		...
		if(message!=null&&message!=""){
			$.post("/positive/update_oa_positive.jr",{
				'message':message,
...
<body>
  ...
  <form enctype="multipart/form-data" id="myform"
    ...
    <div class="form-group" id="tujiaozhi">
      ...
      <div class="col-xs-10">
        ...
        <c:if test="${sessionScope.employee_session.organization_id!=30 }">
         ...
         <c:if test="${sessionScope.employee_session.organization_id==30 }">
          <option value="${sessionScope.employee_session.employee_id }">${sessionScope.employee_session.employee_name }</option>
          </c:if>
        </select>
    ...
    <div class="form-group">
      ...
      <div class="col-xs-10">
      <c:if test="${positive.positive_state==0 && sessionScope.employee_session.organization_id!=30 }">
        <button type="button" onclick="tijiao_positive(${positive.positive_id})" class="btn btn-info btn-xm">提交</button>
       </c:if>
       <c:if test="${positive.positive_state==1 && sessionScope.employee_session.organization_id==30 }">
        <button type="button" onclick="tijiao_positive(${positive.positive_id})" class="btn btn-info btn-xm">提交</button>
       </c:if>
