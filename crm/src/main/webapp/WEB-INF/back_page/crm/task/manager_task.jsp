...
function task_jump_page(page,limit,task_employee_id ,bumen_id ,task_type ,zhongyao_state ,task_finish_time_start_str ,task_finish_time_end_str){
	jiazaidonghua();
	$.post("/crm_task/get_manager_task.jr",{
...
function shaixuan_tioajian(obj,limit){
		...
		$("#myform").ajaxSubmit({
			type : 'POST',
			url : "/crm_task/get_manager_task.jr",
			data:{
				'pageNumber':1,
				'limit':limit
			},
...
function to_send_task(){
	layer.open({
		  ...
		  content: "/crm_task/get_send_task_page.jr"
		  });
}
//查看任务详情
function to_check_task(task_id){
	 layer.open({
		  ...
		  area: ['850px', '500px'],
		  shadeClose: false, //点击遮罩关闭
		  content: "/crm_task/to_check_task.jr?task_id="+task_id
		  });
}
//删除任务信息
function to_delete_task(task_id,limit,page){
	layer.confirm("提示：您好，确定要删除吗？",function(){
		...
		$.post("/crm_task/delete_oa_task.jr",{
			'task_id':task_id
		},function(data){
			if(data==1){
