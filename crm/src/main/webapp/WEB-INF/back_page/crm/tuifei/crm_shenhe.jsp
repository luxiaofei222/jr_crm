...
<script>
//添加联系人信息信息
function tijiao_shenhe(entryInfoId,tuifei_state,role_id){
	if(role_id==16){
		var message =$("#message").val();
		if(message!=null&&message!=""){
		$.post("/crm_tuifei/tuifei_shenqing.jr",{
			'entryInfoId':entryInfoId,
			'tuifei_state':tuifei_state,
			'jingli_tuifei_infeo':message
		},function(data){
			if(data==1){
				tanchuang("申请成功！");
			}else{
...
<div class="shenhe" style="padding: 12px">
	<table width="750" border="1" class="baoming_feiyong">
		...
		<c:if test="${sessionScope.employee_session.role_id==13}">
		<tr class="yijian">
		...
		<tr class="yijian">
			...
			<td colspan="6"><textarea id="message"></textarea></td>
		</tr>
	</table>
	<!--endprint-->
	<div class="caozuo">
	<c:if test="${sessionScope.employee_session.role_id==16 || sessionScope.employee_session.role_id==24 }">
		<input class="btn btn-success btn-xm"
			onclick="tijiao_shenhe(${entryInfo.entryInfoId },2,${sessionScope.employee_session.role_id })" value="同意"
			type="button">
		<input class="btn btn-success btn-xm"
			onclick="tijiao_shenhe(${entryInfo.entryInfoId },6,${sessionScope.employee_session.role_id })" value="拒绝"
			type="button">
	</c:if>
	<c:if test="${sessionScope.employee_session.role_id==13 }">
		<input class="btn btn-success btn-xm"
			onclick="tijiao_shenhe(${entryInfo.entryInfoId },3,${sessionScope.employee_session.role_id })" value="同意"
			type="button">
		<input class="btn btn-success btn-xm"
			onclick="tijiao_shenhe(${entryInfo.entryInfoId },6,${sessionScope.employee_session.role_id })" value="拒绝"
			type="button">
