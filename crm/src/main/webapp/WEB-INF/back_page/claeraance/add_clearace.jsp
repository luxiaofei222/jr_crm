...
<script>
	...
	//添加通关方案
	function add_clearance(obj) {
		if(check_clearance_price()&&check_new_price()){
			$(obj).attr("disabled", "disabled");
			$("#myform").ajaxSubmit({
				type : 'POST',
				url : "/back_clearance/save_clearance.jr",
				success : function(data) {
					if (data == 1) {
						tanchuang("提示：恭喜您，添加通关方案成功！");
						$('#myform')[0].reset();
...
<body>
	<form enctype="multipart/form-data" class="form-horizontal" id="myform" style="padding: 20px;">
      ...
      <div class="col-xs-8">
        <input class="form-control"  id="dengji" name="dengji" placeholder="输入等级，没有区分等级则不填">         
      </div>
    </div>
		<div class="form-group" style="margin-top:15px;">
      <label class="col-xs-3 control-label right_wz">套餐名称：</label>
      <div class="col-xs-8">
        <input class="form-control"  id="clearance_name" name="clearance_name" placeholder="输入套餐名称">         
      </div>
    </div>
    <div class="form-group" style="margin-top:15px;">
      <label class="col-xs-3 control-label right_wz">原价：</label>
      <div class="col-xs-8">
        <input class="form-control"  onblur="check_clearance_price()" id="clearance_price" name="clearance_price" placeholder="输入套餐价格">         
      </div>
    </div>
     <div class="form-group" style="margin-top:15px;">
      <label class="col-xs-3 control-label right_wz">现价：</label>
      <div class="col-xs-8">
        <input class="form-control"  onblur="check_new_price()" id="new_price" name="new_price" placeholder="输入套餐价格">         
      </div>
    </div>
     <div class="form-group" style="margin-top:15px;">
      <label class="col-xs-3 control-label right_wz">课程到期时间：</label>
      <div class="col-xs-8">
        <input class="layui-input" onclick="layui.laydate({elem: this})"   id="daoqi_time_str" name="daoqi_time_str" placeholder="请选择到期时间">         
      </div>
    </div>
    <div class="form-group" style="margin-top:15px;">
      <label class="col-xs-3 control-label right_wz">包含：</label>
      <div class="col-xs-8">
        <input class="form-control" id="clearrance_baohan" name="clearrance_baohan" placeholder="输入套餐包含">         
      </div>
    </div>
    <div class="form-group" style="margin-top:15px;">
      <label class="col-xs-3 control-label right_wz">特色：</label>
      <div class="col-xs-8">
        <textarea class="form-control"  id="clearrance_tese" name="clearrance_tese" placeholder="请输入套餐特色" rows="3" style="color:#444;resize:none;"></textarea>         
      </div>
    </div>
    ...
    <div class="form-group" style="margin-top:5px;">
      ...
      <div class="col-xs-8">
        <input class="form-control"  id="clearrance_baozhang" name="clearrance_baozhang" placeholder="输入套餐保障">         
      </div>
    </div>
		<div class="form-group" style="overflow:hidden;margin-top:20px;">
			...
			<div class="col-xs-6" style="text-align: left;">
				<button type="button" onclick="add_clearance(this)"
