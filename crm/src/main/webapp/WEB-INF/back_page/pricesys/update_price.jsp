...
<script>
	...
	//修改低价体系
	function update_price(price_sys_id) {
		$("#myform").ajaxSubmit({
			type : 'POST',
			url : "/back_price/update_price.jr?price_sys_id="+price_sys_id,
			success : function(data) {
				if (data == 1) {
					tanchuang("提示：恭喜您，修改低价体系成功！");
				} else {
...
<body>
    ...
    <div class="form-group" style="margin-top:15px;">
      ...
      <div class="col-xs-8">
        <input class="form-control" id="price_sys_title" value="${priceSys.price_sys_title }" name="price_sys_title" placeholder="输入标题">         
      </div>
    </div>
    <div class="form-group" style="margin-top:15px;">
      <label class="col-xs-3 control-label right_wz">内容：</label>
      <div class="col-xs-8">
        <textarea class="form-control"  id="price_sys_content" name="price_sys_content" placeholder="低价体系内容" rows="3" style="color:#444;">${priceSys.price_sys_content }</textarea>         
      </div>
    </div>
		<div class="form-group" style="overflow:hidden;margin-top:20px;">
			...
			<div class="col-xs-6" style="text-align: left;">
				<button type="button" onclick="update_price(${priceSys.price_sys_id })"
