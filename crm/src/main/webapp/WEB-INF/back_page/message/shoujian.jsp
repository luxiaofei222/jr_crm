...
function huifu_message(message_id){
	layer.open({
		  type: 2,
		  title: ['回复用户'],
		  area: ['1000px', '650px'],
		  shadeClose: false, //点击遮罩关闭
		  content: "/back_message/to_huifu_message.jr?message_id="+message_id
        });
}
//查看信息详情
...
function message_jump_page(page){
	jiazaidonghua();
	$.post("/back_message/get_shoujian_page.jr",{
		'pageNumber' : page,
		'limit' : 20
	},function(data){
...
function delete_message(){
	var len = $("input:checkbox[name=message_check]:checked").length;
	if(len>0){
		var str = "";
		$('input:checkbox[name=message_check]:checked').each(function(i) {
			if (0 == i) {
				str = $(this).val();
			} else {
				str += ","+$(this).val();
		...
		layer.confirm("提示：您好，确定要删除吗？",function(){
			...
			$.post("/back_message/delete_message.jr",{
				'str':str
			},function(data){
				if(data==1){
					tanchuang("删除成功！");
    ...
    <div class="operation">
       ...
       <div class="opera_right right">
        <select class="form-control select select-primary left top_select" id="select_val" data-toggle="select">
            ...
            <option value="user_name">发件人昵称</option>
          </select>
        ...
        <c:if test="${not empty messages.list }">
            ...
            <td>
              <button type="button" onclick="check_message(${message.message_id })" class="btn btn-inverse btn-xs">查看</button>
               <button type="button" onclick="huifu_message(${message.message_id })" class="btn btn-info btn-xs">回复</button>
            </td>
          </tr>
          </c:forEach>
        </tbody>
