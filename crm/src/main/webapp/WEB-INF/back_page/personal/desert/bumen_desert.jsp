...
//翻页上一页下一页
function desert_jump_page(page,limit,employee_id ,bumen_id ){
	jiazaidonghua();
	$.post("/desert/get_boss_bumen_desert.jr",{
...
//跳转页面
function desert_tiaozhuan_page(page,limit,employee_id ,bumen_id){
...
function shaixuan_tioajian(obj,limit){
		...
		$("#myform").ajaxSubmit({
			type : 'POST',
			url : "/desert/get_boss_bumen_desert.jr",
			data:{
				'pageNumber':1,
				'limit':limit
			},
...
//查看任务详情
function to_check_desert(desert_id){
	 layer.open({
		  type: 2,
		  title: ['离职审批'],
		  area: ['850px', '600px'],
		  shadeClose: false, //点击遮罩关闭
		  content: "/desert/to_check_boss_desert.jr?desert_id="+desert_id
		  });
}
		...
		<div class="content_message">
			<table class="table table-hover">
				...
				<c:if test="${not empty backdeserts.list }">
					<tbody>
						<c:forEach items="${backdeserts.list }" var="desert" varStatus="vs">
							...
							<td><label class="label label-success btn-primary"
								for="minimal-checkbox-1">${vs.index+1+backdeserts.begin }</label></td>
							<td>${desert.employee.employee_name }</td>
							<td><fmt:formatDate 
									value="${desert.desert_time }" /></td>
							<td>${desert.bumen }</td>
							<td>${desert.gangwei }
							</td>
							<td><fmt:formatDate  type="both"
									value="${desert.tijiao_time}" /></td>
							<td>
							<c:if test="${desert.desert_state==0}">
								经理审批中
							</c:if>
							<c:if test="${desert.desert_state==1}">
								总监审批中
							</c:if>
							<c:if test="${desert.desert_state==2}">
								总经理审批中
							</c:if>
							<c:if test="${desert.desert_state==3}">
								未批准
							</c:if>
							<c:if test="${desert.desert_state==4}">
								已通过
							</c:if>
							</td>
							<td>
								<button type="button"
										onclick="to_check_desert(${desert.desert_id })"
			...
			<c:if test="${ empty backdeserts.list }">
				<p class="zanwu" style="text-align: center; color: #53C1FE;">还没有人申请离职哦！</p>
			</c:if>
		</div>
		<div class="pages">
			<div class="whole left">
				<div class="go_page">
					<span>第</span><input type="text" id="page_num" /> <span>页</span> <a
						...
						onclick="desert_tiaozhuan_page(${backdeserts.pages},${limit},'${employee_id }','${bumen_id }')"
						class="btn_go">GO</a>
				</div>
			</div>
			<ul class="pagination right">
				<li><a href="javascript:void(0)"
					onclick="desert_jump_page(1,${limit },'${employee_id }','${bumen_id }')"
					class="fa fa-fast-backward"></a></li>
				<c:if test="${backdeserts.hasPreviousPage==true}">
					<li class="previous"
						onclick="desert_jump_page(${backdeserts.pageNumber-1},${limit },'${employee_id }','${bumen_id }')"><a
						href="#fakelink" class="fa fa-angle-left"></a></li>
				</c:if>
				<c:forEach items="${backdeserts.navigatePageNumbers }" var="page">
					<c:choose>
						<c:when test="${backdeserts.pageNumber==page}">
							<li class="active"><a href="#fakelink">${page}</a></li>
						</c:when>
						<c:otherwise>
							<li
								onclick="desert_jump_page(${page},${limit },'${employee_id }','${bumen_id }')"><a
				...
				<li class="pagination-dropdown dropup"><a href="#fakelink"
					...
					<ul class="dropdown-menu">
						<li><a href="#fakelink"
							onclick="desert_jump_page(${backdeserts.pageNumber},20,'${employee_id }','${bumen_id }')">20</a></li>
						<li><a href="#fakelink"
							onclick="desert_jump_page(${backdeserts.pageNumber},50,'${employee_id }','${bumen_id }')">50</a></li>
						<li><a href="#fakelink"
							onclick="desert_jump_page(${backdeserts.pageNumber},100,'${employee_id }','${bumen_id }')">100</a></li>
					</ul></li>
				<c:if test="${backdeserts.hasNextPage==true}">
					<li class="next"
						onclick="desert_jump_page(${backdeserts.pageNumber+1},${limit },'${employee_id }','${bumen_id }')"><a
						href="#fakelink" class="fa fa-angle-right"></a></li>
				</c:if>
				<li
					onclick="desert_jump_page(${backdeserts.pages},${limit },'${employee_id }','${bumen_id }')"><a
