...
function info_jump_page(page,limit,entryUserName , documentNumber,entryUserPosition ,employee_id ,start_time ,end_time , entryplanId,university_id ,zhuanye_id ,mianshoubanxing ,isPay ,payType ,price,bao_start_time,bao_end_time){
	jiazaidonghua();
	$.post("/fin_entry_info/get_entry_info_main.jr",{
...
function to_examine_page(entryinfo_id){
	layer.open({
		...
		content : "/fin_entry_info/to_examine_page.jr?entryinfo_id="+entryinfo_id
	});
}
...
function shaixuan_tioajian(obj,limit){
		...
		$("#myform").ajaxSubmit({
			type : 'POST',
			url : "/fin_entry_info/get_entry_info_main.jr",
			data:{
				'pageNumber':1,
				'limit':limit
			},
	...
	<div class="right_content">
				...
				<c:if test="${not empty entryInfos.list }">
					<tbody>
						<c:forEach items="${entryInfos.list }" var="info" varStatus="vs">
							...
							<td>${info.payType }</td>
							<td><c:if test="${info.entryInfoState==1 }">
							未审核
							</c:if> <c:if test="${info.entryInfoState==2 }">
							审核成功
							</c:if> <c:if test="${info.entryInfoState==4 }">
							审核未通过
							</c:if></td>
							...
							<td><c:if test="${info.entryInfoState ==1}">
									<button type="button"
										onclick="to_examine_page(${info.entryInfoId })"
										class="btn btn-success btn-xs">审核</button>
								</c:if> <c:if test="${info.entryInfoState !=1}">
									<button type="button"
		...
		<div class="pages">
			...
			<ul class="pagination right">
				...
				<li
					onclick="info_jump_page(${entryInfos.pages},${limit },'${entryUserName }','${ documentNumber}','${entryUserPosition }','${employee_id }','${start_time }','${end_time }','${ entryplanId}','${university_id }','${zhuanye_id }','${mianshoubanxing }','${isPay }','${payType }','${price }','${bao_start_time }','${bao_end_time }')"><a
