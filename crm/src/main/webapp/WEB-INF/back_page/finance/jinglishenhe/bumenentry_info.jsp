...
function info_jump_page(page,limit,entryUserName , documentNumber,entryUserPosition ,employee_id ,start_time ,end_time , entryplanId,university_id ,zhuanye_id ,mianshoubanxing ,isPay ,payType ,price){
	jiazaidonghua();
	$.post("/crm_examine/get_bumenjingli_entryinfo.jr",{
...
function to_examine_page(entryinfo_id){
	layer.open({
		...
		content : "/crm_examine/to_examine_page.jr?entryinfo_id="+entryinfo_id
	});
}
...
function shaixuan_tioajian(obj,limit){
		...
		$("#myform").ajaxSubmit({
			type : 'POST',
			url : "/crm_examine/get_bumenjingli_entryinfo.jr",
			data:{
				'pageNumber':1,
				'limit':limit
			},
				...
				<c:if test="${not empty entryInfos.list }">
					<tbody>
						<c:forEach items="${entryInfos.list }" var="info" varStatus="vs">
							...
							<td>${info.payType }</td>
							<td><c:if test="${info.entryInfoState==7 }">
							未审核
							</c:if>
							</td>
