...
function info_jump_page(page,limit,entryUserName , documentNumber,entryUserPosition ,employee_id ,start_time ,end_time , entryplanId,university_id ,zhuanye_id ,mianshoubanxing ,isPay ,payType ,price){
	jiazaidonghua();
	$.post("/crm_examine/get_shichangzongjian_entryinfo.jr",{
...
function to_examine_page(entryinfo_id){
	layer.open({
		...
		content : "/crm_examine/zongjian_to_examine_page.jr?entryinfo_id="+entryinfo_id
	});
}
...
function shaixuan_tioajian(obj,limit){
		...
		$("#myform").ajaxSubmit({
			type : 'POST',
			url : "/crm_examine/get_shichangzongjian_entryinfo.jr",
			data:{
				'pageNumber':1,
				'limit':limit
			},
