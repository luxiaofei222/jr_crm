...
<script>
	...
	function check_net_conult(consult_id) {
		layer.open({
			...
			content : "/net_conult/to_check_net_conult.jr?consult_id="
					+ consult_id
		});
	}
	
	function to_update_conult(consult_id){
		layer.open({
			type : 2,
			title : [ '修改客户信息' ],
			area : [ '900px', '700px' ],
			shadeClose : false, //点击遮罩关闭
			content : "/net_conult/to_update_conult.jr?consult_id="+consult_id
		});
	}
	//添加客户信息页面弹窗
	function to_save_conult() {
		layer.open({
			type : 2,
			title : [ '添加客户信息' ],
			area : [ '900px', '700px' ],
			shadeClose : false, //点击遮罩关闭
			content : "/net_conult/to_save_conult.jr"
		});
	}
	
	function to_net_conult_track(consult_id){
		layer.open({
			type : 2,
			title : [ '查看客户回访记录' ],
			area : [ '900px', '700px' ],
			shadeClose : false, //点击遮罩关闭
			content : "/net_conult/to_net_conult_track.jr?consult_id="+consult_id
		});
	}
	
	function net_conult_call_record(consult_id){
		layer.open({
			type : 2,
			title : [ '查看客户通话记录' ],
			area : [ '900px', '700px' ],
			shadeClose : false, //点击遮罩关闭
			content : "/net_conult/to_net_conult_call_record.jr?consult_id="+consult_id
		});
	}
	//换单
	function to_update_employee_conult(consult_id){
		layer.open({
			type : 2,
			title : [ '转移客户咨询' ],
			area : [ '500px', '300px' ],
			shadeClose : false, //点击遮罩关闭
			content : "/net_conult/to_update_employee_conult.jr?consult_id="+consult_id
		});
	}
	//评论列表翻页-上一页 下一页
	function conult_jump_page(page, limit, user_sex, user_name, user_phone,
			...
			course_parent_id, review_id, employee_id, bumen_id,
			zixun_start_time, zixun_end_time) {
		jiazaidonghua();
		$.post("/net_conult/get_net_conult_main.jr", {
			'user_sex' : user_sex,
			'user_name' : user_name,
	...
	function conult_tiaozhuan_page(page, limit, user_sex, user_name,
			...
			course_id, course_parent_id, review_id, employee_id, bumen_id,
			zixun_start_time, zixun_end_time) {
		jiazaidonghua();
		$.post("/net_conult/get_net_conult_main.jr", {
			'user_sex' : user_sex,
	...
	//删除评论信息
	function delete_conult(page) {
		var len = $("input:checkbox[name=commentance_check]:checked").length;
		if (len > 0) {
			var str = "";
			$('input:checkbox[name=commentance_check]:checked').each(
					function(i) {
						if (0 == i) {
							str = $(this).val();
						} else {
			...
			layer.confirm("提示：您好，确定要删除吗？", function() {
				...
				$.post("/net_conult/delete_conult.jr", {
					'str' : str
				}, function(data) {
					if (data == 1) {
						tanchuang("删除成功！");
						conult_jump_page(page);
					} else {
						tanchuang("删除失败！");
					}
				});
	...
	function shaixuan_tioajian(obj) {
		...
		$("#myform").ajaxSubmit({
			type : 'POST',
			url : "/net_conult/get_net_conult_main.jr",
			data : {
				'pageNumber' : 1,
				'limit' : 20
			},
	...
	<div class="right_content">
		<form enctype="multipart/form-data" id="myform"
			...
			<div class="form-group">
				...
				<label class="control-label col-xs-1">业务员：</label>
				<div class="col-xs-2">
		...
		<div class="operation">
			<div class="opera_left left">
				<button type="button" onclick="to_save_conult()"
					class="btn left btn-danger" id="save">
					<i class="fa fa-plus"></i>添加
				</button>
				<button type="button"
					onclick="delete_conult(${netConults.pageNumber})"
					class="btn left btn-danger" id="delete_conult">
					<i class="fa fa-minus"></i>删除
				</button>
				<span><b style="margin-left: 15px;line-height: 31px;">总客户数量</b>：<span style="color:#06C1AE;">${all_conult_number }</span></span>
				<span><b style='margin-left:15px;line-height: 31px;'>当前条件数量</b>：<span style="color:orange;">${shai_conult_number }</span></span>
			</div>
			...
			<table class="table table-hover">
				...
				<c:if test="${not empty netConults.list }">
					<tbody>
						<c:forEach items="${netConults.list }" var="conult" varStatus="vs">
							...
							<td><label class="checkbox checkbox-inline"
								for="checkbox${conult.consult_id }"> <input
									type="checkbox" name="commentance_check" data-toggle="checkbox"
									value="${conult.consult_id  }"
									id="checkbox${conult.consult_id  }" class="checkbox" required>
							</label></td>
							...
							<td><c:if test="${empty conult.user_phone }">
							--
							</c:if>${conult.user_phone }</td>
              	...
              	成交
							...
							<td>
							<button type="button"
									onclick="to_net_conult_track(${conult.consult_id  })"
									class="btn btn-warning btn-xs">回访记录</button>
									<button type="button"
									onclick="net_conult_call_record(${conult.consult_id  })"
									class="btn btn-primary btn-xs">通话记录</button>
								<button type="button"
									onclick="check_net_conult(${conult.consult_id  })"
									class="btn btn-success btn-xs">查看</button>
								<button type="button"
									onclick="to_update_conult(${conult.consult_id  })"
									class="btn btn-info btn-xs">修改</button>
								<c:if test="${conult.conult_state==2 }">
								<button type="button"
									onclick="to_update_employee_conult(${conult.consult_id  })"
									class="btn btn-info btn-xs">转单</button>
								</c:if>
							</td>
							</tr>
						</c:forEach>
		...
		<div class="pages">
			<div class="whole left">
				<div class="go_page">
					<button type="button" class="btn btn-success btn-sm selectall"
						style="float: left;">全部选中</button>
					<button type="button" class="btn btn-danger btn-sm selectno"
						style="float: left;">取消全选</button>
					<span>跳转</span><input type="text" id="page_num" /> <span>页</span>
					<a href="javascript:void(0)"
						onclick="conult_tiaozhuan_page(${netConults.pages},${limit},'${user_sex }','${user_name }','${user_phone }','${consult_type }','${search_word }','${hope_sc }','${zhuanye }','${city }','${course_id }','${course_parent_id }','${review_id }','${employee_id }','${bumen_id }','${zixun_start_time }','${zixun_end_time }')"
						class="btn_go">GO</a>
				</div>
			</div>
			<ul class="pagination right">
				<li><a href="javascript:void(0)"
					onclick="conult_jump_page(1,${limit },'${user_sex }','${user_name }','${user_phone }','${consult_type }','${search_word }','${hope_sc }','${zhuanye }','${city }','${course_id }','${course_parent_id }','${review_id }','${employee_id }','${bumen_id }','${zixun_start_time }','${zixun_end_time }')"
					class="fa fa-fast-backward"></a></li>
				<c:if test="${netConults.hasPreviousPage==true}">
					<li class="previous"
						onclick="conult_jump_page(${netConults.pageNumber-1},${limit },'${user_sex }','${user_name }','${user_phone }','${consult_type }','${search_word }','${hope_sc }','${zhuanye }','${city }','${course_id }','${course_parent_id }','${review_id }','${employee_id }','${bumen_id }','${zixun_start_time }','${zixun_end_time }')"><a
				...
				<c:forEach items="${netConults.navigatePageNumbers }" var="page">
					<c:choose>
						...
						<c:otherwise>
							<li
								onclick="conult_jump_page(${page},${limit },'${user_sex }','${user_name }','${user_phone }','${consult_type }','${search_word }','${hope_sc }','${zhuanye }','${city }','${course_id }','${course_parent_id }','${review_id }','${employee_id }','${bumen_id }','${zixun_start_time }','${zixun_end_time }')"><a
				...
				<li class="pagination-dropdown dropup"><a href="#fakelink"
					...
					<ul class="dropdown-menu">
						<li><a href="#fakelink"
							onclick="conult_jump_page(${netConults.pageNumber},20,'${user_sex }','${user_name }','${user_phone }','${consult_type }','${search_word }','${hope_sc }','${zhuanye }','${city }','${course_id }','${course_parent_id }','${review_id }','${employee_id }','${bumen_id }','${zixun_start_time }','${zixun_end_time }')">20</a></li>
						<li><a href="#fakelink"
							onclick="conult_jump_page(${netConults.pageNumber},50,'${user_sex }','${user_name }','${user_phone }','${consult_type }','${search_word }','${hope_sc }','${zhuanye }','${city }','${course_id }','${course_parent_id }','${review_id }','${employee_id }','${bumen_id }','${zixun_start_time }','${zixun_end_time }')">50</a></li>
						<li><a href="#fakelink"
							onclick="conult_jump_page(${netConults.pageNumber},100,'${user_sex }','${user_name }','${user_phone }','${consult_type }','${search_word }','${hope_sc }','${zhuanye }','${city }','${course_id }','${course_parent_id }','${review_id }','${employee_id }','${bumen_id }','${zixun_start_time }','${zixun_end_time }')">100</a></li>
					</ul></li>
				...
				<li
					onclick="conult_jump_page(${netConults.pages},${limit },'${user_sex }','${user_name }','${user_phone }','${consult_type }','${search_word }','${hope_sc }','${zhuanye }','${city }','${course_id }','${course_parent_id }','${review_id }','${employee_id }','${bumen_id }','${zixun_start_time }','${zixun_end_time }')"><a
