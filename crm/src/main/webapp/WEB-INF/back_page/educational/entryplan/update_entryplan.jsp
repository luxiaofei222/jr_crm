...
//添加联系人信息信息
function update_entry_plan(entryplan_id){
	if(check_entryplan_content()&&check_jiezhiriqi()&&check_baomingfei()&&check_jiaocaifei()){
		$("#myform").ajaxSubmit({
			type : 'POST',
			url : "/edu_entry_plan/update_entry_plan.jr?entryplan_id="+entryplan_id,
...
<form class="form-horizontal add_qiye_dialog" enctype="multipart/form-data" id="myform" style="padding:21px;">
	<div class="form-group" style="margin-top: 15px;">
		...
		<div class="col-xs-8">
			<select data-toggle="select" onchange="get_sub_course()"
				...
				<c:if test="${entryPlan.parent_id==cou_menu.course_id }">
	...
	<div class="form-group" style="margin-top: 15px;">
		...
		<div class="col-xs-8">
			<select data-toggle="select" onchange="get_dic_list()" id="course_id"
				...
				<c:if test="${entryPlan.course_id==cou_menu.course_id }">
					<option selected="selected" value="${cou_menu.course_id }">${cou_menu.course_name }</option>
				</c:if>
				<c:if test="${entryPlan.course_id!=cou_menu.course_id }">
					<option value="${cou_menu.course_id }">${cou_menu.course_name }</option>
	...
	<div class="form-group" id="zhicheng_sanji" style="margin-top: 15px;">
		<label class="col-xs-2 control-label right_wz"></label>
		<div class="col-xs-8">
			<select data-toggle="select"  id="review_id"
				...
				<c:if test="${entryPlan.review_id== course.review_id}">
				<option selected="selected" value="${course.review_id }">${course.review_name }</option>
				</c:if>
				<c:if test="${entryPlan.review_id!= course.review_id}">
	...
	<c:if test="${entryPlan.university_id==0 }">
	...
	<c:if test="${entryPlan.university_id!=0}" >
		<div id="chengkao_xuexiao" class="form-group" style="margin-top: 15px;">
		<label class="col-xs-2 control-label right_wz">成考学校：</label>
		<div class="col-xs-8">
			<select data-toggle="select" 
				id="university_id" name="university_id"
				...
				<option value="0">请选择</option>
				<c:forEach items="${chengkaoScs }" var="chengkao">
				<c:if test="${entryPlan.university_id== chengkao.chengkao_id }">
					<option selected="selected" value="${chengkao.chengkao_id }">${chengkao.chengkao_name }</option>
				</c:if>
				<c:if test="${entryPlan.university_id!= chengkao.chengkao_id }">
					<option  value="${chengkao.chengkao_id }">${chengkao.chengkao_name }</option>
				</c:if>
     ...
     <div class="form-group">
      ...
      <div class="col-xs-9">
        <input type="text" value="${entryPlan.entryplan_content }" onblur="check_entryplan_content()" class="form-control" name="entryplan_content" id="entryplan_content" placeholder="请输入报考计划">  
      </div>
    </div>
    <div class="form-group">
      <label class="col-xs-2 control-label">相关说明：</label>
      <div class="col-xs-9">
        <input type="text" class="form-control"value="${entryPlan.entryplan_explain }" name="entryplan_explain" id="entryplan_explain" placeholder="相关说明">
      </div>
    </div>
    <div class="form-group">
      <label class="col-xs-2 control-label">截至日期：</label>
      <div class="col-xs-9">
        <input type="text" class="form-control" onblur="check_jiezhiriqi()" value="${dateString}"   name="jiezhiriqi" id="jiezhiriqi" placeholder="添加截至日期">   
      </div>
    </div>  
       <div class="form-group">
      <label class="col-xs-2 control-label">报名费：</label>
      <div class="col-xs-9">
        <input type="text" class="form-control"  value="${entryPlan.baomingfei}" onblur="check_baomingfei()" name="baomingfei" id="baomingfei" placeholder="报名费">
      </div>
    </div>
     <div class="form-group">
      <label class="col-xs-2 control-label">教材费：</label>
      <div class="col-xs-9">
        <input type="text" class="form-control" value="${entryPlan.jiaocaifei}" onblur="check_jiaocaifei()" name="jiaocaifei" id="jiaocaifei" placeholder="教材费">
      </div>
    </div>
    <c:if test="${entryPlan.parent_id!=18 }">
     ...
     <div class="form-group">
      ...
      <div class="col-xs-7">
        <input type="text" class="form-control" value="${entryPlan.putongbanfei}" name="putongbanfei" id="putongbanfei" placeholder="普通班费用">
      </div>
     </div>
     <div class="form-group">
      ...
      <div class="col-xs-7">
       <input type="text" class="form-control" value="${entryPlan.jingjiangbanfei}" name="jingjiangbanfei" id="jingjiangbanfei" placeholder="精讲班费用">
      </div>
    </div>  
     </div>
    ...
    <div id="xueli" style="display: none;" >
     <div class="form-group">
      ...
      <div class="col-xs-7">
       	<input type="text" class="form-control" value="${entryPlan.chengkao_nian}" name="chengkao_nian" id="chengkao_nian" placeholder="普通班费用">
      </div> --%>
     </div>
    <%--  <div class="form-group">
      <label class="col-xs-2 control-label"></label>
      ...
      <c:if test="${entryPlan.parent_id==18 }">
     <div id="feixueli" style="display: none;">
     <div class="form-group">
      <label class="col-xs-2 control-label">培训费：</label>
      <label class="col-xs-2" style="font-size:13px;text-align:center;">普通班：</label>
      <div class="col-xs-7">
    ...
    <div class="form-group">
      ...
      <div class="col-xs-9">
        <button type="button" onclick="update_entry_plan(${entryPlan.entryplan_id})" class="btn btn-success btn-xm">提交</button>
