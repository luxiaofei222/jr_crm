...
//翻页上一页下一页
function leave_jump_page(page,limit,employee_id ,leave_type,bumen_id ){
	jiazaidonghua();
	$.post("/back_leave/get_boss_leave.jr",{
...
//跳转页面
function leave_tiaozhuan_page(page,limit,employee_id ,leave_type,bumen_id){
...
function shaixuan_tioajian(obj,limit){
		...
		$("#myform").ajaxSubmit({
			type : 'POST',
			url : "/back_leave/get_boss_leave.jr",
			data:{
				'pageNumber':1,
				'limit':limit
			},
...
function to_check_leave(leave_id){
	 layer.open({
		  ...
		  content: "/back_leave/to_check_boss_leave.jr?leave_id="+leave_id
		  });
}
			...
			<div class="whole left">
				<div class="go_page">
					<span>第</span><input type="text" id="page_num" /> <span>页</span> <a
						...
						onclick="leave_tiaozhuan_page(${backLeaves.pages},${limit},'${employee_id }','${leave_type }','${bumen_id }')"
						class="btn_go">GO</a>
				</div>
			</div>
			<ul class="pagination right">
				<li><a href="javascript:void(0)"
					onclick="leave_jump_page(1,${limit },'${employee_id }','${leave_type }','${bumen_id }')"
					class="fa fa-fast-backward"></a></li>
				<c:if test="${backLeaves.hasPreviousPage==true}">
					<li class="previous"
						onclick="leave_jump_page(${backLeaves.pageNumber-1},${limit },'${employee_id }','${leave_type }','${bumen_id }')"><a
				...
				<c:forEach items="${backLeaves.navigatePageNumbers }" var="page">
					<c:choose>
						...
						<c:otherwise>
							<li
								onclick="leave_jump_page(${page},${limit },'${employee_id }','${leave_type }','${bumen_id }')"><a
				...
				<li class="pagination-dropdown dropup"><a href="#fakelink"
					...
					<ul class="dropdown-menu">
						<li><a href="#fakelink"
							onclick="leave_jump_page(${backLeaves.pageNumber},20,'${employee_id }','${leave_type }','${bumen_id }')">20</a></li>
						<li><a href="#fakelink"
							onclick="leave_jump_page(${backLeaves.pageNumber},50,'${employee_id }','${leave_type }','${bumen_id }')">50</a></li>
						<li><a href="#fakelink"
							onclick="leave_jump_page(${backLeaves.pageNumber},100,'${employee_id }','${leave_type }','${bumen_id }')">100</a></li>
					</ul></li>
				...
				<li
					onclick="leave_jump_page(${backLeaves.pages},${limit },'${employee_id }','${leave_type }','${bumen_id }')"><a
