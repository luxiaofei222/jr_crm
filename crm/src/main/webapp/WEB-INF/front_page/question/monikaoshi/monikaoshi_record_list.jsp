				...
				<ul class="records_lineul">
					<li class="records_line1">
						<div class="records_line1_one">
							<span class="records_line1_one1">[模拟考试]</span> <span
								title="${record.zhentiYears.zhenti_year }年<c:if test="${not empty record.zhentiYears.zhenti_yue }">（${record.zhentiYears.zhenti_yue }月）</c:if>${courseMenu.course_name }《${questionCourse.question_course_name }》${record.zhentiYears.zhenti_name }"
								class="records_line1_one2">${record.zhentiYears.zhenti_year }年<c:if test="${not empty record.zhentiYears.zhenti_yue }">（${record.zhentiYears.zhenti_yue }月）</c:if>${courseMenu.course_name }《${questionCourse.question_course_name }》${record.zhentiYears.zhenti_name }
							</span>
						</div>
						...
						<c:if test="${record.recourd_state==1 }">
							<li class="records_line2"><a
								href="/front_zhenti/get_zhenti_question_jiexi.html?question_course_id=${questionCourse.question_course_id}&zhenti_id=${zhenti.zhenti_id }&zhenti_record_id=${zhenti.zhenti_record_id}">查看解析</a>
							</li>
						</c:if>
						<c:if test="${record.recourd_state==2 }">
						...
						<c:if test="${record.recourd_state==1 }">
						 <li class="records_line3">
							<a class="finish"
								href="/front_zhenti/get_zhenti_year_introduce.html?question_course_id=${questionCourse.question_course_id}&zhenti_id=${zhenti.zhenti_id }">再做一次</a>
						</li>
						</c:if>
						<c:if test="${record.recourd_state==2 }">
							<li class="records_line3">
