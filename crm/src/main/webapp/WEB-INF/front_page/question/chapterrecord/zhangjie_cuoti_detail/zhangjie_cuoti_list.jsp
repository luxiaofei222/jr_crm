...
<div class="records_right">
	<c:if test="${empty chapterRecords.list }">
		<div style="height: 250px;">
			<img src="/images/question/chapter/noinfo.png"
				style="margin: 0 auto;">
		</div>
	</c:if>
		...
		<div class="collect_wai">
			<div class="collect_nei">
				<div class="collect_title">
					<span class="num">${vs.index+1+chapterRecords.begin }</span> <span
						class='line'></span> <em class='type'>[${records.question_type }]</em>
					<p>${records.chapterQuestion.question_name }</p>
				</div>
				<c:if test="${records.chapterQuestion.question_type=='单选题' }">
					<ul class="single_choices">
						<c:forEach
							items="${records.chapterQuestion.chapterQuerstionOptions }"
							var="option">
							<c:if
								test="${records.chapterQuestion.answer==option.option_number }">
				...
				<div class="jiucuo">
					<div class="jiucuo_answer">
						...
						<c:if test="${records.is_collection==1 }">
							<span class="collect collect_active"
								onclick="quxiao_question_collection(this,${records.chapter_recourd_id})">
								<i class="icon-star2 collect_active"></i><em>取消收藏</em>
							</span>
						</c:if>
						<c:if test="${records.is_collection==0}">
							<span class="collect collect_active"
								onclick="collection_question(this,${records.chapter_recourd_id})">
								<i class="icon-star2"></i><em>收藏本题</em>
							</span>
						</c:if>
					...
					<div class='fold-bd'>
						<ul class="list-analyze">
							<li><span class='l'>试题难度：</span>
								<div class="r">
									<i class="icon-pen pen-${records.chapterQuestion.difficulty }">
										<span></span>
									</i>
								</div></li>
							<li><span class="l">
									统&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;计： </span>
								<div class='r'>
									该题您答过 <b class="hl-orange">${records.user_answer_number }</b>次，正确率为
									<b class="hl-green"><fmt:formatNumber type="number"
											value="${records.user_answer_right_number/records.user_answer_number*100 }"
											pattern="0.00" maxFractionDigits="2" />%</b>
								</div></li>
							<li><span class="l"> 参考解析： </span>
								<div class="r">${records.chapterQuestion.analysis }</div></li>
						</ul>
					</div>
				</div>
			</div>
	...
	<div class="pagination">
		<c:if test="${chapterRecords.hasPreviousPage==true}">
			<a onclick="zhangjie_cuoti_jump_page(${chapterRecords.pageNumber-1},${question_course_id },${zhang_id },${jie_id },${question_id })">上一页</a>
		</c:if>
		<c:forEach items="${chapterRecords.navigatePageNumbers }" var="page">
			<c:choose>
				...
				<c:otherwise>
					<a onclick="zhangjie_cuoti_jump_page(${page},${question_course_id },${zhang_id },${jie_id },${question_id })">${page}</a>
				</c:otherwise>
			</c:choose>
		</c:forEach>
		<c:if test="${chapterRecords.hasNextPage==true}">
			<a onclick="zhangjie_cuoti_jump_page(${chapterRecords.pageNumber+1},${question_course_id },${zhang_id },${jie_id },${question_id })">下一页</a>
		</c:if>
		<span class="page-num">${chapterRecords.pageNumber}/${chapterRecords.pages }页</span>
		<span class="input"> <input type="text" id="page_number">
		</span> <a onclick="to_zhangjie_cuoti_jump_page(${chapterRecords.pages },${question_course_id },${zhang_id },${jie_id },${question_id })">跳转</a>
	</div>
</div>
